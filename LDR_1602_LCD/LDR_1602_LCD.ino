...
**    Modified to support both 2004 and 1602 LCD display sizes by updating LCDCOLUMNS and LCDROWS constant values
**    3 x 2 font credited to digimike, Arduino forum user
...
/******* Text messages *******/
char msgWelcome1[] = "VxD LDR Passive";              //** <-- maximum 16 characters (line 1)
char msgWelcome2[] = "Preamplifier";       //** <-- maximum 16 characters (line 2)
...
char* setupName[4] = { msgBias, msgMeas, msgCalibM, msgExit };
...
/******* SCREEN *******/
#define LCDBRI_MAX 190       //** LCD full brightness (1 - 255)
#define LCDBRI_MIN 50       //** LCD eco brightness
#define ROW_IN 0            //** LCD row to display input channel (0 - 3)
#if LCDCOLUMNS == 20
#define ROW_OUT 3           //** LCD row to display output channel (0 - 3)
#else
#define ROW_OUT 1           //** LCD row to display output channel (0 - 3)
#endif
...
#define TIME_EXITSELECT 3      //** Time in seconds to exit I/O select mode when no activity
...
#if INPUTCOUNT == 0 && OUTPUTCOUNT == 0 && LCDCOLUMNS == 20
#define VOLCOL 6
#elif INPUTCOUNT == 0 && OUTPUTCOUNT == 0 && LCDCOLUMNS == 16
#define VOLCOL 5
#elif LCDCOLUMNS == 20
#define VOLCOL 11
#else
...
void setMaxResistanceMode() {
  ...
  lcd.setCursor(0, 0);
  if (LCDCOLUMNS == 16) {
    lcd.print("Adjust trimmers");
    lcd.setCursor(0, 1);
    lcd.print("to 700K each LDR");
    delay(3000);
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("then recalibrate");
    lcd.setCursor(0, 1);
...
void setMinResistanceMode() {
  ...
  lcd.setCursor(0, 0);
  if (LCDCOLUMNS == 16) {
    lcd.print("Measure LDR vals");
    lcd.setCursor(0, 1);
    lcd.print("then update code");
    delay(3000);
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("then recalibrate");
...
byte doSelfTest() {
  ...
  lcd.clear();
  lcd.setCursor((LCDCOLUMNS - 2 - sizeof(msgTest)) / 2, 0);
  lcd.print(msgTest);
...
byte doCalibration() {
  ...
  //PRINTLN("Calibration");
  lcd.clear();
  lcd.setCursor((LCDCOLUMNS - 2 - sizeof(msgCalib)) / 2, 0);
  lcd.print(msgCalib);
  tick = 0;
  percent = 0;
  setCalibrationRelays(ON);
...
#if INPUTCOUNT > 2 // inputs are on R1 to R5
...
#else // inputs are on R6
if (chan_in)
mcp.digitalWrite(PIN_EXT_R6, HIGH);
else
mcp.digitalWrite(PIN_EXT_R6, LOW);
#endif
mil_onInput = millis();
}
...
lcd_print(outputName[chan_out], (LCDCOLUMNS / 2) - 1);
...
// Arduino pins the encoder is attached to. Attach the center to ground.  - Already defined
//#define PIN_ENC1 2
//#define PIN_ENC2 3

// Pins should be setup like this (already done in our code setup
...
void toSetupState() {
  ...
  lcd.setCursor(0, 1);
  lcd.write(A); lcd.print(setupName[1]);
  if (LCDCOLUMNS == 20) {
    lcd.setCursor(0, 2);
    lcd.write(A); lcd.print(setupName[2]);
    lcd.setCursor(0, 3);
    lcd.write(A); lcd.print(setupName[3]);
  }
  state = STATE_SETUP;
...
void toErrorState() {
  ...
  setLSH(0); setRSH(0);
  lcd.clear();
  lcd.setCursor(4 + 2 * (LCDCOLUMNS == 20), 0);
  ...
  if (state != STATE_IO) {
    ...
    if (INPUTCOUNT > 1 || OUTPUTCOUNT > 1) {
      if (INPUTCOUNT > 1) {
        ...
        lcd.setCursor(0, ROW_OUT); lcd.write(CURSORCHAR);
      }
    }
    state = STATE_IO;
  }
...
setLCDMaxLight();
    ...
    else if ((INPUTCOUNT > 5 && OUTPUTCOUNT > 2) || (INPUTCOUNT > 2 && OUTPUTCOUNT > 4) || (INPUTCOUNT == 3 && OUTPUTCOUNT == 3)) {
      errc = 30;
      toErrorState();
    }
    else
      ...
      void loop() {
        ...
        if (encoderPos != 0)
        {
          ...
          /** encoder rotated in "setup" mode **/
            ...
            if (LCDCOLUMNS == 20) {
              lcd.setCursor(0, selSetup); lcd.write(A);
              selSetup += encoderPos;
              selSetup %= 4;
            ...
            else
            {
              selSetup += encoderPos;
              selSetup %= 4;
              /** paging of the menu items **/
              if (selSetup == 0) {
                lcd.setCursor(0, 0); lcd.write(CURSORCHAR); lcd_print(setupName[selSetup], String(setupName[selSetup]).length() + LCDCOLUMNS - String(setupName[selSetup]).length() - 1);
                lcd.setCursor(0, 1); lcd.write(A); lcd_print(setupName[selSetup + 1], String(setupName[selSetup + 1]).length() + LCDCOLUMNS - String(setupName[selSetup + 1]).length() - 1);
              }
              if (selSetup == 1) {
                lcd.setCursor(0, 0); lcd.write(A); lcd_print(setupName[selSetup - 1], String(setupName[selSetup - 1]).length() + LCDCOLUMNS - String(setupName[selSetup - 1]).length() - 1);
                lcd.setCursor(0, 1); lcd.write(CURSORCHAR); lcd_print(setupName[selSetup], String(setupName[selSetup]).length() + LCDCOLUMNS - String(setupName[selSetup]).length() - 1);
              }
              if (selSetup == 2) {
                lcd.setCursor(0, 0); lcd.write(CURSORCHAR); lcd_print(setupName[selSetup], String(setupName[selSetup]).length() + LCDCOLUMNS - String(setupName[selSetup]).length() - 1);
                lcd.setCursor(0, 1); lcd.write(A); lcd_print(setupName[selSetup + 1], String(setupName[selSetup + 1]).length() + LCDCOLUMNS - String(setupName[selSetup + 1]).length() - 1);
            ...
            switch (selSetup) {
              ...
              case SEL_EXIT:
                if (!notCalibrated)
                  ...
                  lcd.clear();
                  lcd.setCursor((14 - sizeof(msgNoCalib)) / 2, 1);
